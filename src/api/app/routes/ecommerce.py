...
from ..agents.ecommerce_agent import SemanticKernelEcommerceAgent
from ..schemas.agent import AgentRequest, AgentResponse

router = APIRouter()

logger = logging.getLogger("api.routes.ecommerce")

# Initialize a single agent instance to be used by the router.
agent = SemanticKernelEcommerceAgent()
...
@router.post("/ecommerce/agent", response_model=AgentResponse)
